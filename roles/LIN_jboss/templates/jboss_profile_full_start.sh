...
chown -R jbossadm:jbossgrp /appl/RH
ip=`hostname -i`
START_SERVER="/appl/RH/jboss/bin/standalone.sh -c standalone-full.xml &"
...
start() {
    ...
    sudo -u ${SUDO_JBOSS_USER} -i /appl/RH/jboss/bin/standalone.sh -c standalone-full.xml & >> /appl/RH/jboss/standalone/log/AutoStartupLogs/start.log  2>&1
    sleep 10s
...
restart() {
    ...
    echo "                                                                              " >> /appl/RH/jboss/standalone/log/AutoStartupLogs/restart.log
    sudo -u ${SUDO_JBOSS_USER} -i /appl/RH/jboss/bin/standalone.sh -c standalone-full.xml & >> /appl/RH/jboss/standalone/log/AutoStartupLogs/restart.log  2>&1
    process=$(ps -ef|grep -i java|grep -i jboss)
